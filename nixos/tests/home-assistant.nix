...
in {
  ...
  nodes.hass = { pkgs, ... }: {
    ...
    services.home-assistant = {
     ...
     # test loading custom components
      customComponents = with pkgs.home-assistant.customComponents; [
        prometheus-sensor
      ];

      # test loading lovelace modules
      customLovelaceModules = with pkgs.home-assistant.customLovelaceModules; [
        mini-graph-card
      ];
  ...
  ''
    ...
    with subtest("Check that lovelace modules are referenced and fetchable"):
        hass.succeed("grep -q 'mini-graph-card.js' '${configDir}/ui-lovelace.yaml'")
        hass.succeed("curl --fail http://localhost:8123/local/nixos-lovelace-modules/mini-graph-card.js")
