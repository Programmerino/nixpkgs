...
let
  ...
  customLovelaceModulesResources = {
    lovelace.resources = map (card: {
      url = "/local/nixos-lovelace-modules/${card.pname}.js?${card.version}";
      type = "module";
...
in {
  ...
  options.services.home-assistant = {
    ...
    customLovelaceModules = mkOption {
      ...
      default = [];
      example = literalExpression ''
        with pkgs.home-assistant.lovelaceModules; [
          mini-graph-card
          mini-media-player
        ];
      '';
      description = lib.mdDoc ''
        ...
        Available cards can be found below <package>pkgs.home-assistant.lovelaceModules</package>.

        Only works with lovelace in <literal>yaml</literal> mode.
      '';
    };
